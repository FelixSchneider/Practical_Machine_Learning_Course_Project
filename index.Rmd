...
Because we want to use only those variables for training that are provided for prediction in the *Course Project Prediction Quiz*, we remove the variables that are `NA` in the pml_**testing** set.
...
```
```{r select_notNA_columns}
not_all_NA <- function(x) !all(is.na(x))
quiz2 <- pml_testing %>% select_if(not_all_NA) %>% select(-X)
```
