...
```
```{r remove_NA_columns}
notNA <- colSums(is.na(pml_training))==0
train <- pml_training[,notNA][,-1]
test  <- pml_testing[,notNA][,-1]
```
...
## Features
```{r}
X_train <- select(train,roll_belt:magnet_forearm_z)
y_train <- factor(train$classe)
X_test  <- select(test,roll_belt:magnet_forearm_z)
```
...
prd01RF <- predict(mdl01RF,X_test)
prd01RF
```
